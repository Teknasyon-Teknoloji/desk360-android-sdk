...
class Desk360BottomSheetDialogFragment(val listener: BottomSheetListener) :
    ...
    override fun onCreateView(
        ...
        buttonImage.text =
            Desk360SDK.config?.data?.general_settings?.attachment_images_text
                ?: "Images"
        buttonPdf.text =
            Desk360SDK.config?.data?.general_settings?.attachment_browse_text
                ?: "Document"
        buttonVideo.text =
            Desk360SDK.config?.data?.general_settings?.attachment_videos_text
                ?: "Videos"

        bottomSheetListener = listener

        layout.setBackgroundColor(Color.parseColor(Desk360SDK.config?.data?.general_settings?.main_background_color))
        buttonImage.setTextColor(Color.parseColor(Desk360SDK.config?.data?.general_settings?.header_text_color))
        buttonPdf.setTextColor(Color.parseColor(Desk360SDK.config?.data?.general_settings?.header_text_color))
        buttonVideo.setTextColor(Color.parseColor(Desk360SDK.config?.data?.general_settings?.header_text_color))
